...
<p align="center">
  <em>This section explains how Cheese is powered, how energy moves through the EV3 Brick, why voltage affects robot behavior, and how wiring organization supports reliability during testing.</em>
</p>
...
Cheese depends on stable power to move, steer, read sensors, and use the HuskyLens vision system. The main power source is the **LEGO EV3 rechargeable battery**, which powers the EV3 Brick. From there, the EV3 Brick distributes power to the motors, sensors, Arduino Nano, and HuskyLens system.

For a beginner, the easiest way to understand the power system is this:
...
<p align="center">
  ...
  <strong>The motors, sensors, Arduino Nano, and HuskyLens depend on that power being stable.</strong>
</p>
...
<p align="center">
  <img src="https://img.shields.io/badge/Power_Flow-EV3_Battery_to_All_Main_Systems-FFD43B?style=for-the-badge">
</p>
...
| **Auxiliary lamps** | Extra lighting support if installed. | Use a separate 9V battery so they do not draw power from the EV3 battery. |
...
| **Auxiliary lighting power** | Separate 9V battery if auxiliary lamps are installed |
...
| **Auxiliary lamps** | Separate wiring if installed | External 9V battery | 9V battery supply |
...
<p align="center">
  <em>Power distribution diagram showing how energy flows from the EV3 battery to the main robot systems and how auxiliary lamps use a separate 9V source if installed.</em>
</p>
...
## ❀ 2.1.6 Auxiliary Lighting Power ────୨ৎ────────୨ৎ────

Auxiliary lighting is important because the camera and color sensor depend on what they can see. In previous testing, extra lamps helped reduce lighting problems by making obstacles and floor colors easier to detect.

If auxiliary lamps are installed, they use a **separate 9V battery**. This means the lamps do not draw power from the EV3 battery, so they do not directly reduce the power available for the motors, sensors, Arduino Nano, or HuskyLens.

<div align="center">

| Lighting Element | Power Source | Purpose |
| :--- | :--- | :--- |
| **Auxiliary lamp system** | Separate 9V battery | Supports camera and color visibility when needed. |
| **HuskyLens / camera area** | Powered through Arduino Nano | Detects obstacles and visual targets. |
| **Color sensor area** | EV3 sensor port | Detects floor colors and markers. |

</div>

In the v4 layout, the camera mount became lower and simpler. This reduced the need for bulky upper lighting support and made the robot cleaner. If auxiliary lamps are used again during testing, they should remain electrically separate from the EV3 battery.

<p align="center">
  <img src="../../v-photos/v4/lighting_power_v4.jpg" alt="Cheese v4 auxiliary lighting power setup" width="70%">
</p>

<p align="center">
  <em>Lighting power evidence showing the auxiliary lighting setup and its separate 9V battery if lamps are installed for testing.</em>
</p>

> **Photo note:** If `lighting_power_v4.jpg` appears broken, upload a photo of the auxiliary lighting and 9V battery setup into `v-photos/v4/`.
...
The EV3 Large Motor is responsible for drive, and the EV3 Medium Motor is responsible for steering. Both matter, but the steering motor is especially sensitive during curves because it must reach the commanded angle before the robot reaches the wall.
...
| **Auxiliary lighting setup** | [lighting_power_v4.jpg](../../v-photos/v4/lighting_power_v4.jpg) | Shows the separate 9V power source if auxiliary lamps are installed. |
| **Power distribution diagram** | [power_distribution_diagram_v4.png](../../v-photos/v4/power_distribution_diagram_v4.png) | Shows how power flows from the EV3 battery to the full robot system. |

</div>

This evidence map connects the power explanation to physical documentation. It shows where the EV3, camera, Arduino, wires, and power paths are located or where additional power evidence should be added.
...
| **Auxiliary lighting setup** | ⏳ Pending | [lighting_power_v4.jpg](../../v-photos/v4/lighting_power_v4.jpg) | Separate 9V battery powering auxiliary lamps if installed. |
| **Power distribution diagram** | ⏳ Pending | [power_distribution_diagram_v4.png](../../v-photos/v4/power_distribution_diagram_v4.png) | Overall power flow for the EV3, motors, sensors, Arduino Nano, HuskyLens, and optional lighting. |
...
If auxiliary lamps are installed, they use a separate 9V battery. This keeps lighting support independent from the EV3 battery, which protects the main robot systems from extra electrical load.
